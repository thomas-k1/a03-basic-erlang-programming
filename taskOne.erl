...
taskTwo() ->
      ...
      case checkNumber(Num) of
         continue -> taskTwo();
         stop -> io:format("Program stopped.~n")
      end.
...
checkNumber(Num) when Num > 0 -> 
   FactorialNum = factorial(Num),
   io:format("The factorial of a number that is greater than 0 but not a multiple of 7: ~p~n", [FactorialNum]), 
   continue.
